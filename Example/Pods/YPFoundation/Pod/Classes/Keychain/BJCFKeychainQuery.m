...
@implementation BJCFKeychainQuery
...
#ifdef BJCFKEYCHAIN_SYNCHRONIZATION_AVAILABLE
...
- (BOOL)save:(NSError *__autoreleasing *)error {
	OSStatus status = BJCFKeychainErrorBadArguments;
	...
	CFTypeRef accessibilityType = [BJCFKeychain accessibilityType];
...
- (NSMutableDictionary *)query {
	...
	if ([[self class] isSynchronizationAvailable]) {
		...
		switch (self.synchronizationMode) {
			case BJCFKeychainQuerySynchronizationModeNo: {
			  value = @NO;
			  break;
			}
			case BJCFKeychainQuerySynchronizationModeYes: {
			  value = @YES;
			  break;
			}
			case BJCFKeychainQuerySynchronizationModeAny: {
...
+ (NSError *)errorWithCode:(OSStatus) code {
	...
	switch (code) {
		...
		case BJCFKeychainErrorBadArguments: message = NSLocalizedStringFromTable(@"SSKeychainErrorBadArguments", @"SSKeychain", nil); break;
	...
	return [NSError errorWithDomain:kBJCFKeychainErrorDomain code:code userInfo:userInfo];
}
